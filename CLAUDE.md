...
- **Reusability + design-system inheritance (mandatory):** Every view, component, and Compose UI must be **reusable by construction** and built **from `P*` primitives + `Pasabayan*` tokens**. Before writing a new composable, **search** `:core:designsystem` and the feature's `components/`. Reusable composables take state + callbacks (no `ViewModel`, no DTOs, no business logic), expose a `modifier: Modifier = Modifier`, and ship light + dark previews. The **second time** a composable would be copy-pasted across features, **promote** it to `:core:designsystem` (with tests + previews + spec update) before consuming it. Never fork a `P*` with a one-off bespoke variant — extend the primitive instead.

Cursor rules: `.cursor/rules/design-system-enforcement.mdc` (component + token catalog, workflow when a pattern is missing), `.cursor/rules/ui-reusability-and-inheritance.mdc` (search-before-write, reusable-by-construction, promotion ladder), `.cursor/rules/detail-sheet-scaffold-conformance.mdc` (sheets/modals), `.cursor/rules/compose-ui-previews.mdc` (preview requirements), `.cursor/rules/dark-light-theme-support.mdc` (every UI ships light + dark by default, theme roles + tokens only, no hex literals).
