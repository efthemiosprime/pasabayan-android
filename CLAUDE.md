...
Cursor rules: `.cursor/rules/design-system-enforcement.mdc` (component + token catalog, workflow when a pattern is missing), `.cursor/rules/ui-reusability-and-inheritance.mdc` (search-before-write, reusable-by-construction, promotion ladder), `.cursor/rules/detail-sheet-scaffold-conformance.mdc` (sheets/modals), `.cursor/rules/compose-ui-previews.mdc` (preview requirements).
