...
**Update 2026-05-13:** All seven slices (A–G) in the implementation plan have landed. Carriers can activate planning trips through the sanctioned `POST /trips/{id}/activate`; `EditTripSheet` edits route, schedule, capacity, pricing (with land/non-land branch), and surfaces Activate + Cancel-with-blocking-match handling; `TripStatusUpdateSheet` now exposes the iOS-parity matrix (planning → active/cancelled, active → in_transit/cancelled, in_transit → completed) and routes activate / cancel through the sanctioned endpoints with confirmation alerts; `CreateTripFromPackage` surfaces the request-to-carry alert, auto-preselects the suggested transportation method (`suggested_transportation_method`), and auto-derives shared pickup/delivery from departure/arrival so the wire `pickup_date` / `delivery_date` are populated on create; `TripDetailsScreen` shows a pricing-breakdown visualization when the server returns base/distance/multiplier/calculated. §5 (passenger-mode UI) is closed as a no-op — neither platform renders those dormant fields.
