...
| **4** — Payments & Stripe | … | In progress — slice 1 complete (contract layer) + slice 2 complete (repository/domain hardening: success-flag validation, invalid-payload guards, mapper fallback for transaction status, expanded repository edge-case tests) |
...
| [06-payments-stripe.md](06-payments-stripe.md) | [ ] | Phase 4 slice 1+2 complete: contract + repository/domain layers aligned (Retrofit/DTO, decode fixtures/tests, success-flag and invalid-payload repository guards, mapper fallback coverage); ViewModels/UI remain for next slices |
