...
| Field | Value |
|-------|--------|
| **Current phase** | **Phase 4 — Complete** (Stripe Connect onboarding/dashboard parity) and **Phase 6 — Complete** for profile/verification/favorites/ratings scope. **Phase 5 — Code surface complete** for spec [08](08-notifications-device-tokens.md) (DTOs/repository/lifecycle/VM/router/UI); remaining gate work is dashboard bell wiring + push-tap routing + cross-feature actionable-item aggregation. |
| **Last updated** | 2026-05-11 — Spec [08](08-notifications-device-tokens.md) full surface landed across 6 commits: NotificationApi (7 endpoints, URL-encoded DELETE, raw `ResponseBody` for parser), DTOs (`PushNotificationJson`, `NotificationDataJson` 33 fields, `DeviceTokenRequestJson`/`Response`, `NotificationHistoryResponseJson` + `PaginationInfoJson`, `MarkAllReadDataJson` marked/updated fallback), `UnreadCountResponseParser` (4-shape tolerant), `NotificationType` enum (21 values + icon + color + `@StringRes`), `PushNotification`/`NotificationData`/`PushNotificationRoute` (FCM string→int/double coercion), `NotificationMapper`, `ActionableItemType`, `NotificationPage`, `FCMTokenStore` + `PushNotificationsConsentIntent` (SharedPreferences), `DeviceTokenRegisterResponseParser` (200/201/403 consent/other/decode), `NotificationRepository` with 403→consent retry, `DeviceInfoProvider`, `NotificationChannelSetup` + `NotificationDisplay` (foreground heads-up + `PendingIntent` to `MainActivity` with `EXTRA_NOTIFICATION_DATA`), `NotificationLifecycleManager` (onTokenRefreshed/registerStoredTokenIfAuthenticated/unregisterAndClear) wired into `AuthViewModel.logout` + `MainTabScreen.LaunchedEffect(user)`, `NotificationRouter` SharedFlow + `NotificationRouting.resolve` per-type table, `NotificationViewModel` (per-role parallel unread counts, optimistic mark-as-read with role-count revert, mark-all-read, `onNotificationTapped`, `sendTestNotification`), `PNotificationBadge` + `PUnreadDot` in `:core:designsystem`, `NotificationCard` + `ActionableItemCard` + `NotificationEmptyState` + `NotificationSectionHeader` + `NotificationPendingReviewCard`, `ComprehensiveNotificationsScreen` 3-section sheet, `TimeAgoFormatter` localized, EN/FR `strings_notifications.xml`. **79 JVM tests** (decode + parser + type + mapper + route + parser + repository MockWebServer + lifecycle + routing + router + viewmodel + timeAgo). Phase 4 (spec [06](06-payments-stripe.md)) and Phase 6 (specs [09](09-profile-carrier-consent.md) / [10](10-verification.md) / [11](11-favorites-ratings.md) / [20](20-profile-tab.md)) already closed. |
