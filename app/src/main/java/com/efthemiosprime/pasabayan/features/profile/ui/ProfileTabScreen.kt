...
fun ProfileTabContent(
    ...
    Column(
        ...
        PCard(modifier = Modifier.fillMaxWidth().testTag(ProfileTestTags.Role)) {
            Text(
                text = stringResource(R.string.profile_role_switcher_title),
                style = PasabayanTextStyles.Heading.h5,
            )
            Row(horizontalArrangement = Arrangement.spacedBy(PasabayanSpacing.sm)) {
                Text(
                    text = when (currentRole) {
                        UserRole.CARRIER -> stringResource(R.string.profile_role_chip_carrier)
                        UserRole.SHIPPER -> stringResource(R.string.profile_role_chip_shipper)
                    },
                    style = PasabayanTextStyles.Body.medium,
                )
            }
            PButton(
                text = stringResource(R.string.dashboard_switch_role),
